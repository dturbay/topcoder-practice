...
public class DifferentWaysToAddParentheses {
    ...
    public List<Integer> diffWaysToCompute(String input) {
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            if (ch == '-' || ch == '+' || ch == '*') {
                List<Integer> leftVariants = diffWaysToCompute(input.substring(0, i));
                List<Integer> rightVariants = diffWaysToCompute(input.substring(i + 1));
                for (Integer left: leftVariants) {
                    for (Integer right: rightVariants) {
                        switch (ch) {
                            case '+':
